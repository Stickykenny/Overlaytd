...
@Component({
  ...
  template: `
    <div id="media-docker" class="position-relative" style="z-index: 9999;" cdkDrag>
      <div class="position-absolute top-0 end-0 text-center bg-info bg-gradient p-2 rounded-3" style="max-width:500px;">
        <div *ngIf="isExpanded; then thenBlock; else elseBlock"></div>
        <ng-template #thenBlock>
          <pre>Media Docker</pre>
          <button type="button" class="btn position-absolute top-0 end-0" (click)="onToggleMedia(false)">
        ...
        <ng-template #elseBlock>
          <pre>       Media Docker      </pre>
          <button type="button" class="btn position-absolute top-0 end-0" (click)="onToggleMedia(true)">
        ...
        <div id="nav-wrapper">
          ...
          <div class="tab-content" id="nav-tabContent" style="display:none">
            <div class="tab-pane fade show" id="nav-playlist" role="tabpanel" aria-labelledby="nav-playlist-tab">
              <div id="media-content">
                This Web-application is not affiliated with any of the videos on this Youtube embed
                <iframe
                  height="240"
                  width="480"
                  [src]="iframeYtSource"
            ...
            <div class="tab-pane fade" id="nav-rss" role="tabpanel" aria-labelledby="nav-rss-tab">.....</div>
...
export class MediaDockComponent implements OnInit {
  ...
  constructor(private sanitizer: DomSanitizer) {
    let autoplay: number = BooleanDigit.FALSE;
    let maxVid = 7;
    let rdmVidStart = "&index=" + Math.floor(Math.random() * maxVid);
    this.YtPlaylistId = environment.mediaDockerPlaylistDefault + "&autoplay=" + autoplay + rdmVidStart;
    this.iframeYtSource = this.sanitizer.bypassSecurityTrustResourceUrl(this.YtPlaylistId);
  }
